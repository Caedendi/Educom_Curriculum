...
class ShopModel extends PageModel {
  ...
  public function calculateTotalPrice() {
    ...
    foreach ($this->items as $item) {
      /* JH: Pas op rekenen met floats is gevaarlijk als de het geld representeerd, bij floats is bijvoorbeeld 0.01 + 0.02 != 0.03 maar aan 0.0299999999999999999999999999999, 
             Het is beter om bedragen in centen op te slaan en dan alleen voor presentatie om te zetten naar een float getal */
  ...
  public function getProduct(int $id) {
    ...
    if (!empty($item)) {
      $this->items = new Product(
        (int)$item['id'],
        (string)$item['name'],
        (float)$item['price'], /* JH: Pas op rekenen met floats is gevaarlijk als de het geld representeerd, bij floats is bijvoorbeeld 0.01 + 0.02 != 0.03 maar aan 0.0299999999999999999999999999999, 
                                      Het is beter om bedragen in centen op te slaan en dan alleen voor presentatie om te zetten naar een float getal */
        $item['image'],
        (string)$item['summary'],
  ...
  /* JH: Het is beter om ongeteste code niet te laten staan, het groeit namelijk niet mee met je project en dan is de toestand van de code onbekend. 
         Beter om het te verwijderen, zeker als het al in GIT staat */
